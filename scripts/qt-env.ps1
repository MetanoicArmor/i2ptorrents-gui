...
function Write-CargoQtLocalConfig {
    param(
    ...
    $qtPrefixToml = ($QtPrefix -replace '\\', '/')
    $contents = @"
# Generated by scripts/qt-env.ps1
[env]
QTDIR = "$qtPrefixToml"
CXXFLAGS = "/std:c++17 /Zc:__cplusplus /DNOMINMAX /permissive-"
"@
    $configPath = Join-Path $cargoDir "config.toml.local"
    $utf8NoBom = New-Object System.Text.UTF8Encoding $false
    [System.IO.File]::WriteAllText($configPath, $contents, $utf8NoBom)
}
...
function Initialize-QtCargoEnvironment {
    ...
    if ($qtBin -match '\\msvc') {
        ...
        $env:CXXFLAGS = "/std:c++17 /Zc:__cplusplus /DNOMINMAX /permissive-"
        Write-CargoQtLocalConfig -QtPrefix $prefix
        if ($PersistUserPath) {
