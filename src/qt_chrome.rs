...
extern "C" {
    ...
    fn i2p_apply_app_font(point_size: c_int, fonts_dir: *const c_char, language: *const c_char);
...
pub fn apply_app_font() {
    ...
    let dir_c = dir
        ...
        .map(cstr)
        .unwrap_or_else(|| cstr(""));
    let lang_c = cstr(&language());
    unsafe { i2p_apply_app_font(point_size, dir_c.as_ptr(), lang_c.as_ptr()) }
}
